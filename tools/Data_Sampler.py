...
def generate_clusters_experiments_2(annotations, dataset_path, tables_dir_src,
                                  ...
                                  load_pickle=False):
    logging.info('Creating class counts')
    os.makedirs(save_dir, exist_ok=True)
    if load_pickle:
        filepath = os.path.join(save_dir, 'class_counts.pickle')
        if os.path.isfile(filepath):
    ...
    for linkage in linkage_l:
        for metric in metrics_l:
            for n_clusters in n_clusters_l:
                ...
                copy_annot = copy.deepcopy(annotations)
                labels = cluster_data(scene_fts_l, n_clusters=n_clusters, metric=metric, linkage=linkage, distance_threshold=None)
                logging.info('{}:Initial clutering labels: {}'.format(id_str, len(set(labels))))
                ...
                logging.info('{}:Clusters created are {}'.format(id_str, len(set(train_classes.keys()).union(set(val_classes)))))
                logging.info('{}:They include {}'.format(id_str, set(train_classes.keys()).union(set(val_classes))))
                train_scenes = stratified_sampling_scenes(train_classes, sample_ratio=sample_ratio, random_state=None)
                val_scenes = stratified_sampling_scenes(val_classes, sample_ratio=sample_ratio, random_state=None)

                # Save figures of clusters
                exp_dir = os.path.join(save_dir, id_str)
                os.makedirs(exp_dir, exist_ok=True)
                filepath2 = os.path.join(exp_dir, 'stats.pickle')
                write_pickle((scene_fts_l, labels, list(class_count.keys()), list(class_count_train.keys()), list(class_count_val.keys()), train_scenes, val_scenes), filepath2)
                # Save annotations file
                logging.info('{}: Wrote new annotations file.'.format(id_str))
                copy_annot = remove_scenes(copy_annot, set(train_scenes).union(set(val_scenes)), del_included=False)
                copy_annot['train_split'] = train_scenes
